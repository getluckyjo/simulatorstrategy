"""Get Lucky Golf x Planet Fitness (the Planet Golf concept) - bay economics for the Planet Fitness concession
and the RMB asset-finance facility. Run: python3 model_pf.py  -> writes model_pf.json. Same engine as model.py, Planet Fitness phases and cost base.
...
VA_SHARE = 0.20          # Planet Fitness concession: 20% of bay revenue. Opening position, negotiable
SHELL_PER_BAY = 150_000  # room shell and power if Get Lucky Golf builds it instead of Planet Fitness. Opening position: Planet Fitness builds
...
MEMBERS_PER_CLUB = 3_500   # Planet Fitness does not publish members; assumption, below Planet Fitness's 4,581
ADDON_ZAR_PM = 299       # golf membership add-on on the Planet Fitness bill, ex VAT
...
PHASES = [  # 42 Planet Fitness clubs (51 less the 9 JustGyms)
    dict(name="Phase 1", year=2027, live=0.50, units={"Signature": 20}, clubs=10),   # Signature and Megaclubs, two Signature bays a club
    dict(name="Phase 2", year=2028, live=0.50, units={"Play": 40}, clubs=20),        # two Play bays a club
    dict(name="Phase 3", year=2029, live=0.60, units={"Play": 6, "Practice": 6}, clubs=12),  # one bay a club
]
...
def run(util_mult=1.0, va_share=VA_SHARE, shell=0.0):
    ...
    for y in years:
        ...
        r["people"] = {2027: 1.8e6, 2028: 3.2e6, 2029: 4.4e6}.get(y, 4.8e6)   # 72 bays, not 200: hosts at Signature clubs, two technicians, a small central team
        r["central"] = {2027: 1.2e6, 2028: 1.6e6}.get(y, 2.0e6)
...
def negotiation():
    """2031 outcome for each combination of Planet Fitness's share and who builds the shell."""
    grid = []
    for shell_by, shell in (("Planet Fitness", 0.0), ("Get Lucky Golf", SHELL_PER_BAY)):
...
if __name__ == "__main__":
    ...
    json.dump(out, open("model_pf.json", "w"), indent=1, default=float)
